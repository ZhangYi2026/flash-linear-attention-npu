 ...
 *
 * 约定与上游 PR #19 (flashserve/flash-linear-attention-npu) 新增的
 * `kernel_utils/vector/regbase.hpp` 完全一致 —— 把上游那套 __simd_callee__ 寄存器助手
 * (CastHalf2Float / CastFloat2Half / *FloatTwoReg / LoadIn / HalfOrFloat2Float / ...)
 * 在本文件内"按命名空间内联"一份 (而非放到 kernel_utils 公共路径), 以便:
 *   1) 与 PR #19 的 RegBase 写法/CastTrait/取整模式 (fp32->half 用 CAST_ROUND) 对齐;
 *   2) 自包含, 不与 PR #19 真正合入时新增的公共 regbase.hpp 产生路径冲突。
 *
 ...
 * (跑在 V 流水) 自然组合, 无需额外手工同步 (上游 PR #19 的 vector 驱动同样如此)。
 *
...
// ===== CastTrait (与 PR #19 regbase.hpp 一致) =====
...
// 取整用 CAST_ROUND, 与上游 PR #19 一致 (RegBase 路径的 golden 以此为准)。
...
// ===== 寄存器助手 (与 PR #19 regbase.hpp 同名同义) =====
...
__simd_vf__ inline void NegCastVF(__ubuf__ T* out, __ubuf__ float* in, uint32_t count)
{
    ...
    for (uint16_t i = 0; i < loops; ++i) {
        MaskReg m16 = UpdateMask<T>(cnt);  // fp16 域尾掩码 (递减 cnt); 无尾时退化为满掩码 (= PR #19 行为)
 ...
 * 仿 PR #19 ProcessKKTComputerVF 的 ReduceSum(行和) + 跨行累加(列和)。
 *
    ...
    uint32_t real_BT, uint32_t BT)
{
    ...
    for (uint32_t i = 0; i < real_BT; ++i) {
        ...
        StoreUnAlign(dgcScratch + i, rsReg, uStoreDgc, 1);
        StoreUnAlignPost(dgcScratch + i, uStoreDgc, 0);
...
{
    MaskReg m32 = CreateMask<float, MaskPattern::ALL>();
    constexpr uint32_t flanes = AscendC::VECTOR_REG_WIDTH / sizeof(float);  // 每寄存器 fp32 元素数
    ...
    for (uint16_t b = 0; b < floops; ++b) {
        ...
        StoreUnAlignOut<GType>(dgOut + foff, dgReg, m32, uStoreDg, copyNum);
    }
}
